...
class ActivityManagePage(QWidget):
    ...
    def load_results_to_table(self, results):
        ...
        for row_index, activity in enumerate(results):
            ...
            # 固定欄位順序（必須和 get_all_activities 對齊）
            activity_id, name, start_date, end_date, scheme_names, scheme_items, fee_types, amounts, is_closed = activity

            values = [
                activity_id, name, start_date, end_date,
                scheme_names or "", scheme_items or "",
                fee_types or "", _format_amount_multiline(amounts),
                "已關閉" if str(is_closed).strip() in ("1", "True", "true") else "進行中"
            ]
